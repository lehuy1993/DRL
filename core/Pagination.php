...
class Pagination {
    ...
    function html() {
        ...
        if ($this->_config['total_record'] > $this->_config['limit']) {
            ...
            // Nút prev và first
            if ($this->_config['current_page'] > 1) {
                $p .= '<li><a href="' . $this->__link('1') . ' " class = "prev page-numbers">First</a></li>';
            }
            ...
            for ($i = $this->_config['min']; $i <= $this->_config['max']; $i++) {
                ...
                if ($this->_config['current_page'] == $i) {
                    $p .= '<li><span class = "page-numbers current">' . $i . '</span></li>';
                } else {
                    $p .= '<li><a href="' . $this->__link($i) . '" class = "page-numbers">' . $i . '</a></li>';
                }
            }

            // Nút last và next
            if ($this->_config['current_page'] < $this->_config['total_page']) {
                $p .= '<li><a href="'.$this->__link($this->_config['current_page'] + 1).'" class = "next page-numbers">Next</a></li>';
                $p .= '<li><a href="' . $this->__link($this->_config['total_page']) . '" class = "last page-numbers">Last</a></li>';
            }
    ...
    function htmladmin() {
        ...
        if ($this->_config['total_record'] > $this->_config['limit']) {
            $padmin = '<nav>';
            $padmin = '<ul class = "pagination">';

            // Nút prev và first
            if ($this->_config['current_page'] > 1) {
                $padmin .= '<li><a href="' . $this->__link('1') . ' " title = "Trang đầu"><span aria-hidden = "true">&laquo;</span></a></li>';
            }
            ...
            for ($i = $this ->_config['min']; $i <= $this->_config['max']; $i++) {
                // Trang hiện tại
                if ($this->_config['current_page'] == $i) {
                    $padmin .= '<li class="active"><a href="#">' . $i . ' <span class="sr-only"></span></a></li>';
                } else {
                    $padmin .= '<li><a href="' . $this->__link($i) . '">' . $i . '</a></li>';
                }
            }

            // Nút last và next
            if ($this->_config['current_page'] < $this->_config['total_page']) {
                $padmin .= '<li><a href="'.$this->__link($this->_config['current_page'] + 1).'">Next</a></li>';
                $padmin .= '<li><a href="' . $this->__link($this->_config['total_page']) . '" title = "Trang cuối"><span aria-hidden = "true">&raquo;</span></a></li>';
            }
