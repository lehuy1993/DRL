...
<?php
    ...
    $MSV=isset($_REQUEST['MSV'])?$_REQUEST['MSV']:0;
      $dbsv=new CRUD();
    $id =  $_GET['MSV'];
    $where = array('MSV' =>$id,
                    );
...
<?php
    if (isset($_GET['MSV'])) {
        ...
        $tc5 =isset($_POST['tc5cb'])?$_POST['tc5cb']:'';
        ...
        $data= array(   
         ...
         $numrow = $db-> num_rows();
         if ( $db->delete(Table::tbkqdrl_cb,$where)==true){
       
            $db->insert(TABLE::tbkqdrl_cb,$data);
            $data1 = array('Trangthai'=> 0,);
            $where = array('MSV' =>$MSV );
            $db->update(Table::tbkqdrl_sv,$data1,$where);
            echo "<script> alert('Bạn đã đánh giá xong điểm rèn luyện của $MSV');
        location.href='index.php?options=cham_diem_cb';</script>"; 
    }else{
...
<div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Cán bộ lớp chấm điểm rèn luyện sinh viên <?php echo $row[0]['tenthat'] ?> (<?php echo $_GET['MSV'] ?>)</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
<div class="container">	
...
<div class="panel panel-default">
    ...
    <tbody style="text-align: left;">
        ...
        <tr>
            ...
            <td><input type="radio" class="check_list" name="tc1cb"  value="4"></td>
        </tr>
        <tr>
...
<td><input type="radio" name="tc4_1" disabled="" id="input" value="0"  <?php if ($row[0]['TC4_1']==0) {?> checked="checked"<?php } ?>} ?></td>
        ...
        <input type="submit" name="chamdiem" class="btn btn-primary" value="Gửi điểm"></input> 
        <input style="margin-left: 5px;" type="reset" value="Nhập lại" class="btn btn-warning">
